...
`--seed`: seed для разрешения ничьих (зафиксируйте публично до подсчёта), по умолчанию 0;

`--strict`: требовать строгие полные ранжирования (отклонять равные приоритеты и пропуски);
